...
opencode run -m anthropic/claude-sonnet-4 "Write a test"
opencode run -f src/main.py "Review this file"
opencode run -c "继续上次的会话"
opencode run --attach http://localhost:4096 "Quick question"
opencode run --format json "Return raw JSON events"
opencode run --share "Generate shareable session"
opencode run --thinking "Show thinking blocks"
```

**关键标志**: `-m model`, `-f file`, `-c` (continue), `-s sessionID`, `--fork`, `--attach url`, `--format json|default`, `--share`, `--agent name`, `--dir path`
